...
public class calculator {
    ...
    public static String calc(String input) {
        ...
        if (Character.isDigit(fir.charAt(0)) && Character.isLetter(sec.charAt(0)) || Character.isDigit(sec.charAt(0)) && Character.isLetter(fir.charAt(0))) {
            throw new RuntimeException("Используются одновременно разные системы счисления");
        }
        if (Character.isDigit(fir.charAt(0)) && Character.isDigit(sec.charAt(0))) {
            a = Integer.parseInt(fir);
            b = Integer.parseInt(sec);
        ...
        if (Character.isLetter(fir.charAt(0)) && Character.isLetter(sec.charAt(0))) {
            for (int i = 0; i < 100; i++) {
                if (fir.contentEquals(roman[i])) {
                    a = Integer.parseInt(arabic[i]);
                }
                if (sec.contentEquals(roman[i])) {
                    b = Integer.parseInt(arabic[i]);
                }
            }
            if (a < 1 || a > 10 || b < 1 || b > 10) {
            ...
            for (int i = 0; i < 100; i++) {
                if (result.contentEquals(arabic[i])) {
                    result = roman[i];
                    break;
                }
            }
